$(document).ready(function () {
    ...
    /* Draw data table */
    dtable = serverSideDatatable(route('fs.index'), invisibleCols);
        ...
        multiDeleteBtn.on('click', function () {
            if (selectedObjects.length > 0) {
                showYesNoModal(swlTitle, swlMultiDeleteText, swlIcon, function () {
                    let exeUrl = route(`fs.massDestroy`),
                        data = {schedules: selectedObjects};
    ...
    $(document).on('change', '.status-checkbox', function () {
        ...
        let schedule = self.attr('data-id'),
            exeUrl = route('fs.massUpdate'),
            data = {
                schedules: [schedule],
                fields: {
                    status: self.val()
            ...
            if (selectedObjects.length > 0) {
                showYesNoModal(swlTitle, swlText, swlIcon, function () {
                    ...
                    showEnableDisableAlert(
                        ...
                        0,
                        function (newStatus) {
                            let exeUrl = route('fs.massUpdate'),
                                data = {
                                    schedules: selectedObjects,
                                    fields: {
                                        status: newStatus
                                    }
                                };
