$(document).ready(function () {
    ...
    dtable = serverSideDatatable(route('static-pages.index'), invisibleCols);
        ...
        multiDeleteBtn.on('click', function () {
            if (selectedObjects.length > 0) {
                showYesNoModal(swlTitle, swlMultiDeleteText, swlIcon, function () {
                    let exeUrl = route(`static-pages.massDestroy`),
                        data = {pages: selectedObjects};
    ...
    $(document).on('change', '[dmovie-switch-dt]', function () {
        ...
        let page = self.attr('data-id'),
            exeUrl = route('static-pages.massUpdate'),
            data = {
                pages: [page],
                fields: {
                    status: self.val()
            ...
            if (selectedObjects.length > 0) {
                showYesNoModal(swlTitle, swlText, swlIcon, function () {
                    ...
                    showEnableDisableAlert(
                        ...
                        0,
                        function (newStatus) {
                            let exeUrl = route('static-pages.massUpdate'),
                                data = {
                                    pages: selectedObjects,
                                    fields: {
                                        status: newStatus
                                    }
                                };
